...
const metadataService = dependencyFactory.get<IMetadataService>(
    SERVICES.IMetadataService,
);
...
const dataSourceService = dependencyFactory.get<IDataSourceService>(
    SERVICES.IDataSourceService,
);
...
const DataLoadPageComponent: React.FC = () => {
    ...
    const init = async () => {
        let elementOptionsResult = await metadataService.getAllAsOptions(
            messageService!,
        );
    ...
    const onSelectedElementChangeHandler = async () => {
        ...
        let newType = elementOptions.find((x) => x.value === elementId)?.data
            .type;

        if (newType === 'entity') {
            let newElement = await entityTypeService.get(
                elementId,
                messageService!,
            );
        ...
        if (newType === 'link') {
            let newElement = await linkTypeService.get(
                elementId,
                messageService!,
            );
            setElement(newElement);
    ...
    const onUploadHandler = async () => {
        ...
        while (offset < file.size) {
            ...
            var text: any = await processChunkAsync(chunk);
            if (result.id)
                await dataSourceService.writeFile(
                    result.id,
                    text,
                    messageService!,
                );
