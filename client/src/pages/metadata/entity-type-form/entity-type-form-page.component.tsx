...
const EntityTypeFormPageComponent: React.FC = () => {
    ...
    // Component State
    const { id } = useParams();
    const [entityType, setEntityType] = useReducer(formReducer<IEntityType>, {
    ...
    const init = async (id: string | undefined) => {
        ...
        setParentOptions(
            (
                ...
                return { ...x, disabled: x.value === entityType.id };
            }), // stops the user from selecting the same type as the parent
        );
    };

    const handleParentUpdate = async () => {
        entityType.properties = entityType.properties?.filter(
            (x) => !x.inherited,
        );

        if (entityType.parent?.id) {
            const parent = await entityService.get(
            ...
            setEntityType({
                ...
                properties: [
                    ...
                    ...parent.properties.map((x) => {
                        return { ...x, inherited: true };
                    }),
                ],
            });
        }
    ...
    const checkDefinitionValidity = () => {
        if (!entityType.name || !entityType.pluralName) return false;
    ...
    const onSaveHandler = async () => {
        await entityService.create(entityType, messageService!).then((x) => {
            ...
            <Panel id="definition" header="Definition" className="mb-4">
                ...
                <div className="row">
                    <div className="col-md-6">
                        <div className="form-group">
                            <label
                                htmlFor="entityType.description"
                                className="font-bold block mb-2">
                                Description
                            </label>
                            <InputTextarea
                    ...
                    <div className="col-md-6">
                        <div className="form-group">
                            <label
                                htmlFor="entityType.keywords"
                                className="font-bold block mb-2">
                                Keywords
                            </label>
                            <KeywordSearch
                                value={entityType.keywords}
                                valueSetter={(e) =>
                                    setEntityType({
                                        ...entityType,
                ...
                <div className="row justify-content-end px-3">
                    <div className="col" style={{ maxWidth: '80px' }}>
                        <Button
                            label="Save"
                            disabled={!(definitionValid && propertiesValid)}
