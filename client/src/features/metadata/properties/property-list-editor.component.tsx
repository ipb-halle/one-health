...
import { IPropertyInfo } from './models/property';
import './property-list-editor.component.scss';
...
import { CollectionPlaceholderComponent } from '../../../shared/components';
...
const PropertyListEditorComponent: React.FC<PropertyListProps> = ({
    properties,
    safe,
    parentUpdate,
    mode,
    ...
    const dataTypeBodyTemplate = (property: IPropertyInfo) => {
        if (property.dataType)
            return (
                <FontAwesomeIcon
                    icon={dataTypesToIcons[property.dataType]}
                    size="lg"
                />
            );
        return <></>;
    ...
    const dataTypeOptionTemplate = (option: any) => {
        return (
            <FontAwesomeIcon icon={dataTypesToIcons[option.value]} size="lg" />
        );
    };

    const isKeyBodyTemplate = (property: IPropertyInfo) => {
    ...
    const getActionButtonsTemplate = (property: IPropertyInfo) => {
        if (!property.id)
            return (
                <i
                    className="pi pi-trash"
                    style={{ color: 'red' }}
                    onClick={onRemovePropertyHandler(property.name)}></i>
            );
        return <></>;
    };
    ...
    const onAddPropertyHandler = () => {
        ...
        if (properties.findIndex((x) => x.name === newProperty) >= 0) {
            messageService?.show({
                severity: 'error',
                summary: 'Error',
                detail: `Can't have two properties with the same name ${newProperty}`,
            });
            return;
        }
    ...
    const headerIcons = () => {
        return (
            <Button
                ...
                onClick={onAddPropertyHandler}></Button>
        );
    };
    ...
    const groupHeaderTemplate = (data: IPropertyInfo) => {
        return (
            <div className="flex align-items-center gap-2">
                {data.inherited && <span className="font-bold">Inherited</span>}
                {!data.inherited && (
                    <span className="font-bold">Non-inherited</span>
                )}
            </div>
        );
    };
    ...
    const onCellEditComplete = (e: ColumnEvent) => {
        ...
        if (rowData.inherited) {
            // Inherited properties of the parent can't be modified
            // TODO: BUG: When editing Name and Description I get the error toast twice
            messageService?.show({
                severity: 'error',
                summary: 'Error',
                detail: "Inherited properties can't be modified.",
            });
            return;
        }
        ...
        if (field === 'name') {
            ...
            if (properties.filter((x) => x.name === newValue)?.length > 1) {
                messageService?.show({
                    severity: 'error',
                    summary: 'Error',
                    detail: "Can't have two properties with the same name",
                });
                rowData[field] = originalName;
                return;
            }
        ...
        if (field === 'key') {
            if (safe) {
                messageService?.show({
                    severity: 'error',
                    summary: 'Error',
                    detail: "Can't modify the keys of stored definitions.",
                });
                return;
            }
            ...
            if (rowData.inherited) {
                messageService?.show({
                    severity: 'error',
                    summary: 'Error',
                    detail: "Can't modify inherited keys.",
                });
                return;
            }
        }
        ...
        if (field === 'dataType') {
            if (rowData.id) {
                // The user can't modify the data type of a property that has already been saved
                messageService?.show({
                    severity: 'error',
                    summary: 'Error',
                    detail: "Can't modify the data type of a property that has been saved before.",
                });
                return;
            }
        }
        ...
        return (
            <Panel
                ...
                <DataTable
                    ...
                    groupRowsBy="inherited"
                    rowGroupHeaderTemplate={groupHeaderTemplate}
                    sortField="inherited"
                    scrollable
                    scrollHeight="400px"
                    tableStyle={{ lineHeight: '10px' }}
                    style={{ height: '100%' }}>
                    <Column
                        field="dataType"
                        header=""
                        body={dataTypeBodyTemplate}
                        style={{ paddingRight: '2px', width: '20px' }}
                        editor={(options) => dataTypeEditor(options)}
...
export default PropertyListEditorComponent;
