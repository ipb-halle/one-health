...
class CytoscapeInteractiveChartComponent extends Component<CytoscapeInteractiveChartProps> {
    ...
    addElement(element: any) {
        ...
        if (result.length > 0) {
            this.messageService?.show({
                severity: 'error',
                summary: 'Error',
                detail: 'The node is already in the network',
            });
            return;
        }
        ...
        if (cytoscapeCore) {
            ...
            if (contextMenu) {
                cytoscapeCore.cxtmenu({
                    selector: 'node',
                    menuRadius: (ele: any) => {
                        return 45 * (2 - cytoscapeCore.zoom());
                    },
                    ...
                    commands: [
                        ...
                        },
                        {
                            ...
                            select: (node: any) => {
                                const inNodes = cytoscapeCore
                                    .elements()
                                    .jsons()
                                    .filter(
                                        (x: any) =>
                                            x.group === 'edges' &&
                                            x.data.source === node.id(),
                                    )
                                    .map((x: any) => x.data.target);

                                const outNodes = cytoscapeCore
                                    .elements()
                                    .jsons()
                                    .filter(
                                        (x: any) =>
                                            x.group === 'edges' &&
                                            x.data.target === node.id(),
                                    )
                                    .map((x: any) => x.data.source);

                                inNodes.forEach((id: string) => {
                                    if (!this.lockedNodes.has(id))
                                        cytoscapeCore.remove(
                                            `node[id="${id}"]`,
                                        );
                                });
                        ...
                        },
                        {
                            ...
                            select: async (node: any) => {
                                ...
                                const graph =
                                    await this.graphService.getNodeExpansion(
                                        node.id(),
                                        nodes,
                                        this.messageService!,
                                    );
    ...
    styles: cytoscape.Stylesheet = {
        ...
        style: {
            backgroundColor: (ele) => {
                return darkenHexColor(ele.data('color'), -140);
            },
            width: 75,
            height: 75,
            'border-width': '3px',
            ...
            }, //TODO: add maximum lenght
            'text-valign': 'center',
            'text-halign': 'center',
            color: (ele: any) => {
                return 'black';
            }, //{return getContrastColor(darkenHexColor(ele.data("color"), -140))}, TODO: make this conditional to the background
            'overlay-opacity': 0,
    ...
    edgeStyle: cytoscape.Stylesheet = {
        ...
        style: {
            ...
            'line-color': () => {
                return this.props.edgeLineColor;
            },
            label: (ele: any) => {
                return ele.data('label');
            },
            ...
            'target-arrow-shape': (ele) => {
                return ele.data('direction') !== 'UNDIRECTED'
                    ? 'triangle'
                    : 'none';
            },
