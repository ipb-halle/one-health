...
import { ITypeQuery } from '../models/type-query';
...
class TypeQueryBuilder extends Component<
    TypeQueryBuilderProps,
    TypeQueryBuilderState
> {
    ...
    constructor(props: TypeQueryBuilderProps) {
        super(props);
        this.entityService = dependencyFactory.get<IEntityTypeService>(
    ...
    async initWidgets() {
        this.options = await this.entityService.getAllEntityTypesAsOptions(
            this.messageService!,
        );
    }
    ...
    async onTypeChangedHandler() {
        if (this.state.query.type) {
            var entityType = await this.entityService.get(
            ...
            var filters = properties.map((x) => {
                return { property: x.name, value: undefined };
            });
    ...
    render() {
        return (
            <div
                ...
                <div
                    ...
                    <Dropdown
                        ...
                        onChange={async (e) => {
                            ...
                            var filters = entityType.properties
                                .sort((a, b) => a.position! - b.position!)
                                .map((x) => {
                                    return {
                                        property: x.name,
                ...
                <ScrollPanel
                    style={{ padding: 2, height: this.props.height - 53 }}>
                    {!this.state.query.type && (
